...
export default function Navbar() {
  ...
  const navLinkStyle = "px-3 py-2 text-sm text-slate-300 hover:bg-slate-800 hover:text-white rounded-lg transition-colors duration-200";
    ...
    <header className="sticky top-0 z-50 bg-slate-900/50 backdrop-blur-sm border-b border-slate-700/50">
      <nav className="max-w-7xl mx-auto flex justify-between items-center px-4 sm:px-6 py-3">
        <div className="flex items-center gap-4 md:gap-6">
          <Link href="/" className="text-xl sm:text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-500">
            CoinLens
          </Link>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div className="md:hidden">
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="text-slate-300 hover:text-white p-1 rounded-md">
      ...
      {isMenuOpen && (
        <div className="md:hidden px-4 pb-4 space-y-2 border-t border-slate-700/50">
